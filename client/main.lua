...
Citizen.CreateThread(function()
    if DCHub.Framework == "oldqb" then 
        while QBCore == nil do
            TriggerEvent('QBCore:GetObject', function(obj) QBCore = obj end)
            Citizen.Wait(200)
        end
	elseif DCHub.Framework == "qb" then
		while QBCore == nil do
            Citizen.Wait(200)
        end
    end
    playTime = GetGameTimer() + (DCHub.NeededPlayTime * 60000)
	PlayerData = QBCore.Functions.GetPlayerData()
    Wait(1000)
    SendNUIMessage({
        type = 'translate', 
        translate = DCHub.Language,
    })		
end)
...
RegisterCommand(DCHub.OpenCommand, function()
	openMenu()
end)
...
function openMenu()
    if openMenuSpamProtect < GetGameTimer() then 
        ...
        QBCore.Functions.TriggerCallback("dc-playTimeShop:getPlayerDetails", function(result)
            ...
            SendNUIMessage({
                type = 'openui', 
                coin = result.coin,
                categories = DCHub.Categories,
                items = DCHub.Items,
                steamid = steamID,
                firstname = firstname,
                lastname = lastname,
                remaining = remainingTime,
                coinReward = DCHub.RewardCoin,
                coinList = DCHub.CoinList,
...
Citizen.CreateThread(function()
    while true do
        ...
        if checkTime <= 0 then
            playTime = GetGameTimer() + (DCHub.NeededPlayTime * 60000)
            TriggerServerEvent('dc-playTimeShop:addCoin', DCHub.RewardCoin)
        end
    end
end)
...
RegisterNUICallback('buyItem', function(data, cb)
    if buyItemSpamProtect < GetGameTimer() then 
        ...
        QBCore.Functions.TriggerCallback("dc-playTimeShop:buyItem", function(result)
            cb(result)
        end, data)
    end
end)
...
RegisterNUICallback('sendInput', function(data, cb)
    if sendInputProtect < GetGameTimer() then 
        ...
        QBCore.Functions.TriggerCallback("dc-playTimeShop:sendInput", function(result)
            cb(result)
        end, data)
    end
end)
...
CreateThread(function()
    while true do
        ...
        if IsControlJustPressed(1, 199) then
            ExecuteCommand("coinshop")
        end
    end
  end)
  

RegisterNetEvent('radial:coinshop')
AddEventHandler('radial:coinshop', function()
